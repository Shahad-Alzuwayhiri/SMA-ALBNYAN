...
/**
 * Contract Model - نموذج العقد
 * Handles all database operations related to contracts
 * يتعامل مع جميع عمليات قاعدة البيانات المتعلقة بالعقود
 */
class Contract
{
    private $db; // Database connection - اتصال قاعدة البيانات
    
    /**
     * Constructor - initialize database connection
     * المنشئ - تهيئة اتصال قاعدة البيانات
     */
    ...
    /**
     * Find contract by ID - البحث عن عقد بالمعرف
    ...
    public function getAllForManager($filters = [])
    {
        ...
        $stmt = $this->db->prepare("
            ...
            LEFT JOIN notifications n ON c.id = n.related_contract_id AND n.is_read = 0
            WHERE {$whereClause}
            GROUP BY c.id
    ...
    public function update($id, $data, $userId = null)
    {
        ...
        $allowedFields = [
            ...
            'amount', 'profit_percentage', 'contract_date', 'contract_duration',
            'notes', 'terms_conditions', 'status', 'reviewed_by', 'signed_by',
    ...
    {
        // توليد رقم عقد فريد
        return $this->generateUniqueContractNumber();
    }
    ...
    private function generateUniqueContractNumber()
    {
        ...
        // البحث عن آخر رقم عقد في نفس الشهر
        $stmt = $this->db->prepare("SELECT contract_number FROM contracts WHERE contract_number LIKE ? ORDER BY id DESC LIMIT 1");
        $pattern = "SMA-{$year}{$month}-%";
        ...
        // تنسيق الرقم مع الأصفار البادئة
        $sequenceNumber = str_pad($newNumber, 4, '0', STR_PAD_LEFT);
        
        return "SMA-{$year}{$month}-{$sequenceNumber}";
    }
