<?php
// BASE_URL: computed from SCRIPT_NAME, stripping trailing /public occurrences
$d = dirname($_SERVER['SCRIPT_NAME'] ?? '/');
$root = preg_replace('#(/public)+$#', '', rtrim($d, '/')) . '/';
if (!defined('BASE_URL')) define('BASE_URL', $root);

// asset($path): returns full public URL to an asset or page
if (!function_exists('asset')) {
    function asset($path) {
        // normalize incoming path and ensure we don't duplicate 'public/' segments
        $p = ltrim($path, '/');
        return BASE_URL . 'public/' . $p;
    }
}
